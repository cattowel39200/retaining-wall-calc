...
export interface WallInput {
  ...
  key_depth: number
  key_width: number
  key_x: number  // 저판 좌단 기준 키 중심 x좌표 (m)
}
...
export function formToInput(f: WallFormFields): WallInput {
  ...
  return {
    ...
    rebar1_spacing: f.rebar1_spacing ?? 0,
    rebar2_dia: f.rebar2_dia,
    rebar2_spacing: f.rebar2_spacing ?? 0,
    rebar3_dia: f.rebar3_dia ?? f.rebar1_dia,
    rebar3_spacing: f.rebar3_spacing ?? f.rebar1_spacing ?? 0,
    rebar_toe_dia: f.rebar_toe_dia ?? f.rebar1_dia,
    rebar_toe_spacing: f.rebar_toe_spacing ?? f.rebar1_spacing ?? 0,
